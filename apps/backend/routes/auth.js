...
router.post('/login', async (req, res) => {
    try {
        ...
        console.log('User found:', user.custom_user_id, '|', user.email);
...
router.post('/logout', authenticateToken, async (req, res) => {
    try {
        ...
        // Invalidate all sessions for this user (more secure)
        await pool.query(
            'UPDATE user_sessions SET is_active = false WHERE user_id = $1',
            [userId]
        );

        // Also invalidate specific refresh token if provided
        if (refreshToken) {
            await pool.query(
        ...
        // Destroy express-session (for attendance pages)
        if (req.session) {
