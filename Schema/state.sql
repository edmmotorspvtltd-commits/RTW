-- PostgreSQL Database Schema for Country, State, and City
-- Optimized for cascading dropdowns with autocomplete/search functionality
-- Created: 2026-01-08
...
-- Index for autocomplete search on country name
CREATE INDEX idx_countries_name ON countries(name);
CREATE INDEX idx_countries_name_lower ON countries(LOWER(name));
CREATE INDEX idx_countries_name_pattern ON countries(name varchar_pattern_ops);
...
-- Indexes for autocomplete search and foreign key performance
CREATE INDEX idx_states_name ON states(name);
CREATE INDEX idx_states_name_lower ON states(LOWER(name));
CREATE INDEX idx_states_name_pattern ON states(name varchar_pattern_ops);
CREATE INDEX idx_states_country_id ON states(country_id);

-- Composite index for faster filtering by country
...
-- Indexes for autocomplete search and foreign key performance
CREATE INDEX idx_cities_name ON cities(city);
CREATE INDEX idx_cities_name_lower ON cities(LOWER(city));
CREATE INDEX idx_cities_name_pattern ON cities(city varchar_pattern_ops);
CREATE INDEX idx_cities_state_id ON cities(state_id);

-- Composite index for faster filtering by state
CREATE INDEX idx_cities_state_name ON cities(state_id, city);
...
-- ============================================================
-- Sample Queries for Autocomplete/Dropdown
-- ============================================================

-- Query 1: Get all countries for initial dropdown
-- SELECT id, name, country_code FROM countries ORDER BY name;

-- Query 2: Search countries by name (autocomplete)
-- SELECT id, name, country_code FROM countries 
-- WHERE LOWER(name) LIKE LOWER('%search_term%') 
-- ORDER BY name LIMIT 10;

-- Query 3: Get states by country_id
-- SELECT id, name FROM states 
-- WHERE country_id = ? 
-- ORDER BY name;

-- Query 4: Search states by name and country (autocomplete)
-- SELECT s.id, s.name, c.name as country_name 
-- FROM states s 
-- JOIN countries c ON s.country_id = c.id 
-- WHERE s.country_id = ? AND LOWER(s.name) LIKE LOWER('%search_term%') 
-- ORDER BY s.name LIMIT 10;

-- Query 5: Get cities by state_id
-- SELECT id, city FROM cities 
-- WHERE state_id = ? 
-- ORDER BY city;

-- Query 6: Search cities by name and state (autocomplete)
-- SELECT c.id, c.city, s.name as state_name 
-- FROM cities c 
-- JOIN states s ON c.state_id = s.id 
-- WHERE c.state_id = ? AND LOWER(c.city) LIKE LOWER('%search_term%') 
-- ORDER BY c.city LIMIT 10;

-- Query 7: Get full location hierarchy
-- SELECT 
...
-- ============================================================
-- Optimization - Run ANALYZE to update statistics
-- NOTE: Run VACUUM manually outside of transaction if needed
-- ============================================================
ANALYZE countries;
ANALYZE states;
ANALYZE cities;
...
-- Schema Information
-- ============================================================
-- Database: location_db (or your preferred name)
-- Total Tables: 3 (countries, states, cities)
-- Total Records: 250 countries + 36 states (India) + 603 cities (India)
-- 
-- Key Features:
-- 1. Foreign Key Constraints with CASCADE operations
-- 2. Indexes for fast search/autocomplete (name, LOWER(name), pattern matching)
-- 3. Composite indexes for country+state and state+city queries
-- 4. Timestamps for audit trail
-- 5. SERIAL for auto-increment primary keys
-- 
-- Performance Notes:
-- - varchar_pattern_ops index helps with LIKE queries
-- - LOWER(name) index helps with case-insensitive search
